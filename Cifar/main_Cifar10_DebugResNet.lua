...
cmd:option('-dataset','./data/cifar10_original.t7','')
cmd:option('-model','res_BN','')
...
cmd:option('-save',"log_Cifar10" ,'subdirectory to save logs')
...
cmd:option('-weightDecay',0.0001,'weight Decay for regularization')
...
cmd:option('-depth',110,'Depth of the residual network')
cmd:option('-hidden_number',48,'')
cmd:option('-BNScale',1,'')
...
cmd:option('-modelSave',1,'')
cmd:option('-condition_interval',  1,'the step to debug the weights')
...
baseString=opt.model..'_depth'..opt.depth
         ...
         ..'_b'..opt.batchSize
         ..'_wD'..opt.weightDecay
         ..'_lD'..opt.learningRateDecay..'_seed'..opt.seed

         
log_name='Cifar10_'..baseString
opt.save=opt.save..'/'..log_name
print('log saving: '..opt.save)
...
function train()
  ...
  for t,v in ipairs(indices) do
    ...
    optim[opt.optimMethod](function(x)
      ...
      print(string.format("Iter: %6s,  loss = %6.6f, lr=%6.6f", iteration,loss_Iter, opt.learningRate))            
...
function test()
  ...
  for i,v in ipairs(data_split) do
     local output_tmp=model:forward(v)
    -- print(output_tmp)
     confusion:batchAdd(output_tmp, labels_split[i])
  end
...
------------------------------conditonNumber and eigValue------------
 conditionNumber_gradOutputs={}
 conditionNumber_inputs={}
 maxEig_gradOutputs={}
 maxEig_inputs={}
 eig_gradOutputs={}
 eig_inputs={}
 ...
 for k,v in pairs(model:findModules('nn.Linear_AllMode')) do
        print('------------------match:Linear----------')
       table.insert(conditionNumber_gradOutputs,v.conditionNumber_gradOutput)
       table.insert(conditionNumber_inputs,v.conditionNumber_input)
       table.insert(maxEig_gradOutputs, v.maxEig_gradOutput)
       table.insert(maxEig_inputs, v.maxEig_input)
       table.insert(eig_gradOutputs, v.eig_gradOutput)
       table.insert(eig_inputs, v.eig_input)
       table.insert(weightNorms, v.norm_weight)
  end
     

    results.conditionNumber_gradOutputs=conditionNumber_gradOutputs
    results.conditionNumber_inputs=conditionNumber_inputs
    results.maxEig_gradOutputs=maxEig_gradOutputs
    results.maxEig_inputs=maxEig_inputs
    results.eig_gradOutputs=eig_gradOutputs
    results.eig_linear_inputs=eig_linear_inputs
    results.weightNorms=weightNorms
    torch.save('result_Cifar10_DebugResNet_'..baseString..'.dat',results)
end
if opt.modelSave==1 then
  torch.save('SModel_Cifar10_'..baseString..'.t7', net:clearState())
end
